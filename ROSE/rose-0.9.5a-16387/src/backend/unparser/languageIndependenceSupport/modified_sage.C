...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isOperator (SgExpression* expr)
{
...
void
Unparse_MOD_SAGE::curprint (std::string str)
{
  unp->cur << str;
}
...
// DQ (8/13/2007): Added by Thomas to refactor unparser.
void
Unparse_MOD_SAGE::curprint_newline ()
{
...
// DQ (8/13/2007): Added by Thomas to refactor unparser.
int
Unparse_MOD_SAGE::cur_get_linewrap ()
{
...
// DQ (8/13/2007): Added by Thomas to refactor unparser.
void
Unparse_MOD_SAGE::cur_set_linewrap (int nr)
{
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isBinaryEqualsOperator (SgExpression* expr)
{
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isBinaryInequalityOperator (SgExpression* expr)
{
  SgFunctionRefExp* func_ref = isSgFunctionRefExp (expr);
  SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (expr);
  ...
  if (func_name == "operator<=" || func_name == "operator>=" || func_name
      == "operator<" || func_name == "operator>" || func_name == "operator!=")
    return true;
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isBinaryArithmeticOperator (SgExpression* expr)
{
  SgFunctionRefExp* func_ref = isSgFunctionRefExp (expr);
  SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (expr);
  ...
  if (func_name == "operator+" || func_name == "operator-" || func_name
      == "operator*" || func_name == "operator/" || func_name == "operator+="
      || func_name == "operator-=" || func_name == "operator*=" || func_name
      == "operator/=")
    return true;
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isBinaryParenOperator (SgExpression* expr)
{
  SgFunctionRefExp* func_ref = isSgFunctionRefExp (expr);
  SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (expr);
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isBinaryBracketOperator (SgExpression* expr)
{
  SgFunctionRefExp* func_ref = isSgFunctionRefExp (expr);
  SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (expr);
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isBinaryOperator (SgExpression* expr)
{
  ...
  if (func_name == "operator+" || func_name == "operator-" || func_name
      ...
      == "operator&=" || func_name == "operator|=" || func_name == "operator<<"
      || func_name == "operator>>" || func_name == "operator>>=" || func_name
      == "operator<<=" || func_name == "operator==" || func_name
      == "operator!=" || func_name == "operator<=" || func_name == "operator>="
      || func_name == "operator&&" || func_name == "operator||" || func_name
      == "operator," || func_name == "operator->*" || func_name == "operator->"
...
Unparse_MOD_SAGE::isUnaryOperator (SgExpression* expr)
{
  ...
  // If it is a non-member function this it will have a single argument
  // ROSE_ASSERT(isSgFunctionRefExp(expr) == NULL);
  if (isSgFunctionRefExp (expr) != NULL && isSgMemberFunctionRefExp (expr)
      == NULL)
  {
  ...
  if (isUnaryOperatorPlus (mfunc_ref) || isUnaryOperatorMinus (mfunc_ref)
      || func_name == "operator!" ||
  // func_name == "operator*" ||
      isUnaryDereferenceOperator (mfunc_ref) ||
  // DQ (11/24/2004): Added support for address operator "operator&"
      // func_name == "operator&" ||
      isUnaryAddressOperator (mfunc_ref) || func_name == "operator--"
      || func_name == "operator++" || isUnaryOrOperator (mfunc_ref) ||
  // func_name == "operator~")
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isUnaryPostfixOperator (SgExpression* expr)
{
  // DQ (5/6/2007): This might be a non-member function and if so we don't handle this case correctly!
  ROSE_ASSERT(isSgFunctionRefExp(expr) == NULL);
  ...
  if (mfunc_ref != NULL)
  {
    ...
    if (mfunc_sym != NULL)
    {
      ...
      {
        SgName func_name = mfunc_decl->get_name ();
        if (func_name.getString () == "operator++" || func_name.getString ()
            == "operator--")
        {
          SgInitializedNamePtrList argList = mfunc_decl->get_args ();
...
GetOperatorVariant (SgExpression* expr)
{
  ...
  if (func_name == "operator,")
    ...
    return V_SgDotExp;
  else if (func_name.find ("operator") == string::npos || func_name
      == "operator()")
    return V_SgFunctionCallExp;
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::PrintStartParen (SgExpression* expr, SgUnparse_Info& info)
{
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::RemovePareninExprList (SgExprListExp* expr_list)
{
...
Unparse_MOD_SAGE::isOneElementList (SgConstructorInitializer* con_init)
{
  ...
  {
    printf (
        "In Unparse_MOD_SAGE::isOneElementList: con_init->get_args() == NULL \n");
  }
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::printConstructorName (SgExpression* expr)
{
        ...
        while (true)
        {
          ...
          if ((*i)->variant () == FUNC_CALL)
          {
            ...
            if (isOperator (func_call->get_function ()))
            {
              ...
              SgFunctionRefExp* func_ref = isSgFunctionRefExp (
                  func_call->get_function ());
              SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (
                  func_call->get_function ());
              SgType* return_type;
              if (func_ref)
...
Unparse_MOD_SAGE::isUnaryOperatorArrowSubtree (SgExpression* expr)
{
  ...
  if (returnType != NULL)
  {
    SgFunctionCallExp* functionCall = isSgFunctionCallExp (
        returnType->get_operand ());
  ...
  else
  {
    ...
    if (functionCall != NULL)
    {
      ...
      if (binaryOperator != NULL)
      {
        ...
        if (isOverloadedArrowOperator (binaryOperator->get_rhs_operand ())
            == true)
        {
          returnValue = true;
        }
...
Unparse_MOD_SAGE::NoDereference (SgExpression* expr)
{
  ...
  if (deref_exp != NULL)
  {
    SgFunctionCallExp* func_call = isSgFunctionCallExp (
        deref_exp->get_operand ());
    if (func_call != NULL)
    {
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isIOStreamOperator (SgExpression* expr)
{
  SgFunctionRefExp* func_ref = isSgFunctionRefExp (expr);
  SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (expr);
  if (!func_ref && !mfunc_ref)
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::isCast_ConstCharStar (SgType* type)
{
  SgPointerType* pointer_type = isSgPointerType (type);
  if (pointer_type != NULL)
  {
    SgModifierType* modifier_type = isSgModifierType (
        pointer_type->get_base_type ());
    if (modifier_type != NULL
        && modifier_type->get_typeModifier ().get_constVolatileModifier ().isConst ())
    {
...
//-----------------------------------------------------------------------------------
bool
Unparse_MOD_SAGE::noQualifiedName (SgExpression* expr)
{
  SgMemberFunctionRefExp* mfunc_ref = isSgMemberFunctionRefExp (expr);
  if (mfunc_ref != NULL)
  {
...
Unparse_MOD_SAGE::directives (SgLocatedNode* lnode)
{
  ...
  printf (
      "Unparse_MOD_SAGE::directives(): commented out call to output_pragma() \n");
...
//-----------------------------------------------------------------------------------
void
Unparse_MOD_SAGE::printSpecifier1 (SgDeclarationStatement * decl_stmt,
    SgUnparse_Info & info)
{
  // only valid in class
  ...
  if (info.CheckAccess ())
  {
    ...
    // Adam Betts : in Fortran some declaration statements have undefined access
    // modifiers. Added this case to below, otherwise an assert in the unparser fails
    ...
    // Adam Betts : had to comment this as well
    //ROSE_ASSERT(info.isUnsetAccess() == false);
  }
}

void
Unparse_MOD_SAGE::outputExternLinkageSpecifier (
...
void
Unparse_MOD_SAGE::outputTemplateSpecializationSpecifier (
    SgDeclarationStatement* decl_stmt)
{
#if 0
...
void
Unparse_MOD_SAGE::printSpecifier2 (SgDeclarationStatement* decl_stmt,
    SgUnparse_Info& info)
{
...
#pragma message ("Non-GNU backends not specifically addressed, assuming g++ version 3.4.x compliant ...")
  ...
  if (functionDeclaration != NULL)
  {
    ...
    isDeclarationOfTemplateSpecialization = (specializationEnumValue
    ...
    if (functionDeclaration->get_functionModifier ().hasOpenclWorkGroupSizeHint ())
    {
      SgFunctionModifier::opencl_work_group_size_t
          opencl_work_group_size =
              functionDeclaration->get_functionModifier ().get_opencl_work_group_size ();
  ...
  if (decl_stmt->get_declarationModifier ().get_storageModifier ().get_thread_local_storage ()
      == true)
  {
    curprint ("__thread ");
  }
...
void
Unparse_MOD_SAGE::printSpecifier (SgDeclarationStatement* decl_stmt,
    SgUnparse_Info & info)
{
...
void
Unparse_MOD_SAGE::setupColorCodes (
...
void
Unparse_MOD_SAGE::printColorCodes (SgNode* node, bool openState, vector <pair <
    bool, std::string> > & stateVector)
{
  // This function is part of test to embed color codes (or other information) into the unparsed output.
