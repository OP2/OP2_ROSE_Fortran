    ...
    SgStatement* stmt, SgUnparse_Info& info)
{
  ...
  if (fortranCompilerName == "gfortran")
  {
            ...
            && (BACKEND_FORTRAN_COMPILER_MINOR_VERSION_NUMBER <= 1)))
    {
      ...
      printf (
          "Warning: gfortran versions before 4.2 can not handle absolute path names in the Fortran specific include mechanism (using basename)... \n");

      includeFileName = StringUtility::stripPathFromFileName (
          includeLine->get_filename ());
    }
  }
  else
  {
    ...
    printf (
        "Default compiler behavior ... in code generation (Fortran include uses absolute paths) \n");
    // ROSE_ASSERT(false);
  }
#endif
    ...
    SgInitializedName* initializedName, SgUnparse_Info& info)
{
  ...
  if (info.SkipBaseType () == false)
  {
    ...
    // CARLO (08/02/2011): added device attribute to enable CUDA Fortran kernel generation
    ...
    // CARLO (26/04/2011): added shared attribute to enable CUDA Fortran kernel generation
    ...
    // CARLO (26/04/2011): added constant attribute to enable CUDA Fortran kernel generation
