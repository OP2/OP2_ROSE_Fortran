    ...
    SgLabelRefExp* numeric_label_exp, SgUnparse_Info& info)
{
  ...
  if (numeric_label_exp != NULL)
  {
    ...
    if (numeric_label >= 0)
    {
      ...
      string numeric_label_string = StringUtility::numberToString (
          numeric_label);
    ...
    SgStatement* stmt, SgUnparse_Info& info)
{
  ...
  switch (stmt->variantT ())
  {
    ...
    {
      printf (
          "FortranCodeGeneration_locatedNode::unparseLanguageSpecificStatement: Error: No unparse function for %s (variant: %d)\n",
...
void
FortranCodeGeneration_locatedNode::unparseFormatItemList (
...
unparseDimensionStatementForArrayVariable (SgPntrArrRefExp* arrayReference)
{
  ...
  SgVarRefExp* variableReference = isSgVarRefExp (
    ...
    switch (variableScope->variantT ())
    {
      case V_SgBasicBlock:
      {
        ...
        SgStatementPtrList::iterator i = find (statementList.begin (),
            statementList.end (), variableDeclaration);
...
unparseDimensionStatement (SgStatement* stmt)
{
  ...
  while (i != parameterList.end ())
  {
    ...
    bool unparseForArrayVariable = unparseDimensionStatementForArrayVariable (
        arrayReference);
...
void
FortranCodeGeneration_locatedNode::unparseAttributeSpecificationStatement (
    SgStatement* stmt, SgUnparse_Info& info)
{
  SgAttributeSpecificationStatement* attributeSpecificationStatement =
      isSgAttributeSpecificationStatement (stmt);
...
#define IntentSpecBase 600
    ...
    switch (attributeSpecificationStatement->get_intent ())
    {
      ...
      {
        printf (
            "Error: default reached attributeSpecificationStatement->get_intent() = %d \n",
  ...
  else
  {
    ...
    if (nameList.empty () == true)
    {
      ...
      if (SgProject::get_verbose () >= 1)
        printf (
            "***** WARNING: in unparser NON \"IMPLICT NONE\" STATEMENT NOT YET FINISHED (skipped, default implicit type rules apply) *****\n");
    }
    else
    {
...
void
FortranCodeGeneration_locatedNode::unparseStatementFunctionStmt (
    SgStatement* stmt, SgUnparse_Info& info)
{
    ...
    SgUnparse_Info& info)
{
  ...
  while (i != vardecl->get_variables ().end ())
  {
    ...
    if (isSameVariant == true && arrayType != NULL)
    {
      ...
      SgArrayType* arrayTypeBaseType = isSgArrayType (
          ...
          == arrayType->get_rank ())
      {
        ...
        if (previousArrayType->get_rank () == arrayType->get_rank ())
        {
          ...
          // printf ("isSameVariant = %s \n",isSameVariant ? "true" : "false");
#else
          ROSE_ASSERT (arrayTypeDimensionList->get_expressions ().size ()
              == previousArrayTypeDimensionList->get_expressions ().size ());
          ...
          do
          {
                ...
                == previousArrayTypeDimensionList->get_expressions ()[i]->variantT ())
            {
              ...
              if ((isSgColonShapeExp (
                  arrayTypeDimensionList->get_expressions ()[i]) == NULL)
...
#endif
              {
                ...
                string
                    arrayTypeDimensionListString =
                        arrayTypeDimensionList->get_expressions ()[i]->unparseToString (
                            &info);
                string
                    previousArrayTypeDimensionListString =
                        previousArrayTypeDimensionList->get_expressions ()[i]->unparseToString (
                            &info);
#if 0
...
getElseIfStatement (SgIfStmt* parentIfStatement)
{
  ...
  SgBasicBlock* falseBlock = isSgBasicBlock (
      parentIfStatement->get_false_body ());
  // printf ("falseBlock = %p \n",falseBlock);
  if (falseBlock != NULL)
  {
    ...
    SgUnparse_Info& info)
{
  ...
  // We might want to fix this by having an IR node to represent the Fortran "if" statement.
  // unparseStatement(if_stmt->get_conditional(), info);
  SgExprStatement* expressionStatement = isSgExprStatement (
  ...
  else
  {
    ...
    else
    {
      ...
      SgStatementPtrList & statementList = isSgBasicBlock (
          if_stmt->get_true_body ())->get_statements ();
      ROSE_ASSERT (statementList.size () == 1);
      SgStatement* statement = *(statementList.begin ());
      ROSE_ASSERT (statement != NULL);
    ...
    SgUnparse_Info& info)
{
  ...
  SgExprStatement* conditionStatement = isSgExprStatement (
...
FortranCodeGeneration_locatedNode::unparse_IO_Support (SgStatement* stmt,
    bool skipUnit, SgUnparse_Info& info)
{
  // Sage node corresponds to Fortran IO control info
  SgIOStatement* io_stmt = isSgIOStatement (stmt);
  ROSE_ASSERT (io_stmt != NULL);
...
FortranCodeGeneration_locatedNode::unparse_IO_Control_Support (string name,
    SgExpression* expr, bool isLeadingEntry, SgUnparse_Info& info)
{
  if (expr != NULL)
  {
    ...
    SgUnparse_Info& info)
{
  ...
  else
  {
    ...
    unparse_IO_Control_Support ("FMT", readStatement->get_format (), false,
        info);
    ...
    unparse_IO_Control_Support ("NML", readStatement->get_namelist (), false,
        info);
    unparse_IO_Control_Support ("ADVANCE", readStatement->get_advance (),
        false, info);
    ...
    SgUnparse_Info& info)
{
  ...
  unparse_IO_Control_Support ("NLT", writeStatement->get_namelist (), false,
      info);
  ...
  // F2003 specific
  unparse_IO_Control_Support ("ASYNCHRONOUS",
      writeStatement->get_asynchronous (), false, info);
    ...
    SgUnparse_Info& info)
{
  ...
  // F90 specific
  unparse_IO_Control_Support ("POSITION", openStatement->get_position (),
      false, info);
  ...
  // F2003 specific
  unparse_IO_Control_Support ("ASYNCHRONOUS",
      openStatement->get_asynchronous (), false, info);
    ...
    SgUnparse_Info& info)
{
  ...
  else
  {
    ...
    unparse_IO_Control_Support ("ACCESS", inquireStatement->get_access (),
        false, info);
    ...
    unparse_IO_Control_Support ("EXIST", inquireStatement->get_exist (), false,
        info);
    unparse_IO_Control_Support ("OPENED", inquireStatement->get_opened (),
        false, info);
    unparse_IO_Control_Support ("NUMBER", inquireStatement->get_number (),
        false, info);
    ...
    unparse_IO_Control_Support ("SEQUENTIAL",
        inquireStatement->get_sequential (), false, info);
    unparse_IO_Control_Support ("DIRECT", inquireStatement->get_direct (),
        false, info);
    unparse_IO_Control_Support ("FORMATTED",
        inquireStatement->get_formatted (), false, info);
    unparse_IO_Control_Support ("UNFORMATTED",
        inquireStatement->get_unformatted (), false, info);
    ...
    unparse_IO_Control_Support ("POSITION", inquireStatement->get_position (),
        false, info);
    unparse_IO_Control_Support ("ACTION", inquireStatement->get_action (),
        false, info);
    ...
    unparse_IO_Control_Support ("READWRITE",
        inquireStatement->get_readwrite (), false, info);
    unparse_IO_Control_Support ("DELIM", inquireStatement->get_delim (), false,
        info);
    unparse_IO_Control_Support ("PAD", inquireStatement->get_pad (), false,
        info);

    // F2003 specific
    unparse_IO_Control_Support ("ASYNCHRONOUS",
        inquireStatement->get_asynchronous (), false, info);
    unparse_IO_Control_Support ("DECIMAL", inquireStatement->get_decimal (),
        false, info);
    unparse_IO_Control_Support ("STREAM", inquireStatement->get_stream (),
        false, info);
...
void
FortranCodeGeneration_locatedNode::unparseBackspaceStatement (
    SgStatement* stmt, SgUnparse_Info& info)
{
  // Sage node corresponds to Fortran input/output statement
...
void
FortranCodeGeneration_locatedNode::unparseAssociateStatement (
    SgStatement* stmt, SgUnparse_Info& info)
{
  // Sage node corresponds to Fortran input/output statement
...
void
FortranCodeGeneration_locatedNode::unparseFuncArgs (
...
void
FortranCodeGeneration_locatedNode::unparseInitNamePtrList (
    SgInitializedNamePtrList* args, SgUnparse_Info& info)
{
...
FortranCodeGeneration_locatedNode::unparseVarDecl (SgStatement* stmt,
    SgInitializedName* initializedName, SgUnparse_Info& info)
{
  ...
  if (info.SkipBaseType () == false)
  {
    ...
    if (variableDeclaration->get_declarationModifier ().get_storageModifier ().isCudaDevice ()
        == true)
    {
      curprint (", DEVICE");
    }
...
void
FortranCodeGeneration_locatedNode::printDeclModifier (
    SgDeclarationStatement* decl_stmt, SgUnparse_Info & info)
{
  printf (
      "Access modifiers are handled differently for Fortran, this function printDeclModifier() should not be called! \n");
  ROSE_ASSERT (false);
...
void
FortranCodeGeneration_locatedNode::printAccessModifier (
    SgDeclarationStatement * decl_stmt, SgUnparse_Info & info)
{
  // FIXME: this will look different for full-featured Fortran

  printf (
      "Access modifiers are handled differently for Fortran, this function printAccessModifier() should not be called! \n");
  //   ROSE_ASSERT(false);
...
void
FortranCodeGeneration_locatedNode::unparseBindAttribute (
...
void
FortranCodeGeneration_locatedNode::printStorageModifier (
    SgDeclarationStatement* decl_stmt, SgUnparse_Info& info)
{
  // FIXME: this will look different for full-featured Fortran

  printf (
      "Access modifiers are handled differently for Fortran, this function printStorageModifier() should not be called! \n");
  ROSE_ASSERT (false);
    ...
    SgUnparse_Info& info)
{
  ...
  SgProcedureHeaderStatement* procedureHeader = isSgProcedureHeaderStatement (
      stmt);
...
void
FortranCodeGeneration_locatedNode::unparseFunctionArgs (
...
//-----------------------------------------------------------------------------------
void
FortranCodeGeneration_locatedNode::unparse_helper (
    SgFunctionDeclaration* funcdecl_stmt, SgUnparse_Info& info)
{
  ROSE_ASSERT (funcdecl_stmt != NULL);
...
#else
  printf (
      "In FortranCodeGeneration_locatedNode::unparse_helper(): Skipping name qualification for now, likely we WILL need this in Fortran 90 specific cases later \n");
#endif
...
void
FortranCodeGeneration_locatedNode::unparseClassDeclStmt_derivedType (
    SgStatement* stmt, SgUnparse_Info& info)
{
  ...
  else
  {
    if (!info.inEmbeddedDecl ())
    {
      ...
      SgClassDefinition *cdefn = isSgClassDefinition (
          classdecl_stmt->get_parent ());
        ...
        == true)
    {
      ...
      {
        printf (
            "Warning: statement marked as public in non-module scope in FortranCodeGeneration_locatedNode::unparseClassDeclStmt_derivedType(). \n");
      }
    }
...
void
FortranCodeGeneration_locatedNode::unparseClassDeclStmt_module (
    SgStatement* stmt, SgUnparse_Info& info)
{
  // SgClassDeclaration* classdecl_stmt = isSgClassDeclaration(stmt);
    ...
    SgUnparse_Info& info)
{
  ...
  if (info.SkipBasicBlock () == false)
  {
    ...
    {
      curprint (string ("\n#pragma pack(") + StringUtility::numberToString (
          packingAlignment) + string (")"));
    }
...
void
FortranCodeGeneration_locatedNode::unparseDeallocateStatement (
    SgStatement* stmt, SgUnparse_Info& info)
{
...
FortranCodeGeneration_locatedNode::curprint (const std::string & str) const
{
  ...
  if (is_fortran90 && curr_line_len != 0 && (str_len + curr_line_len)
      > MAX_F90_LINE_LEN)
  {
    unp->u_sage->curprint ("&");
    unp->cur.insert_newline (1);
...
void
FortranCodeGeneration_locatedNode::unparseOmpBeginDirectiveClauses (
    SgStatement* stmt, SgUnparse_Info& info)
{
  ROSE_ASSERT (stmt != NULL);
  // optional clauses
  ...
  {
    const SgOmpClausePtrList& clause_ptr_list = isSgOmpClauseBodyStatement (
        stmt)->get_clauses ();
...
void
FortranCodeGeneration_locatedNode::unparseOmpEndDirectiveClauses (
    SgStatement* stmt, SgUnparse_Info& info)
{
  ROSE_ASSERT (stmt != NULL);
  // optional clauses
...
void
FortranCodeGeneration_locatedNode::unparseOmpEndDirectivePrefixAndName (
    SgStatement* stmt, SgUnparse_Info& info)
{
  ROSE_ASSERT (stmt != NULL);
  unp->u_sage->curprint_newline ();
