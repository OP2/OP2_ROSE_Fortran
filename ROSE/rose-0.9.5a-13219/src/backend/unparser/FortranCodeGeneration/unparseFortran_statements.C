  ...
  // printf ("In unparseVarDecl(SgStatement,SgInitializedName,SgUnparse_Info): info.SkipBaseType() = %s \n",info.SkipBaseType() ? "true" : "false");
     if (info.SkipBaseType() == false )
       ...
       // DQ (3/23/2008): Likely POINTER should also be handled in the unparsing of the type!
					...
					if (variableDeclaration->get_declarationModifier().get_typeModifier().isShared() == true)
					{
						curprint(", SHARED");
					}

					// CARLO (26/04/2011): added constant attribute to enable CUDA Fortran kernel generation
					if (variableDeclaration->get_declarationModifier().get_typeModifier().isConstant() == true)
					{
						curprint(", CONSTANT");
					}
