...
CPPOpenCLUserSubroutine::createStatements ()
{
    ...
    if (isVariableDeclaration == NULL)
    {
      ...
      if (isSgImplicitStatement (*it) != NULL)
      {
            ...
            != originalParameters->get_args ().end (); ++paramIt)
        {
          ...
          if (parallelLoop->isIndirect (opDatCounter) && parallelLoop->isRead (
              opDatCounter))
          {
  ...
  SgVariableDeclaration
      * globalConstantsParameter =
          CPPOpenCLStatementsAndExpressionsBuilder::appendVariableDeclarationAsFormalParameter (
              OpenCL::CPP::globalConstants, buildOpaqueType (
                  OpenCL::CPP::globalConstantsType, subroutineScope),
